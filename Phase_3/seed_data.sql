...
-- Seed data for Midtown Motors (Phase 3).
-- Load after schema.sql. To reset: DROP DATABASE, recreate, then load schema + this file.
--
-- Data overview:
--   13 vehicles: 7 sold, 6 unsold (4 ready for sale, 2 with pending parts)
--   6 staff users: 2 acq specialists, 2 sales agents, 1 manager, 1 owner (all roles)
--   6 customers: 5 individual, 1 business
--   2 vendors, 5 parts orders with various statuses
--
-- Key test scenarios covered:
--   - Multi-color vehicle (7SUV — White + Black)
--   - Same-day sale (6COUPE)
--   - Heavy parts cost triggering seller history highlight (8TRUCK via C4)
--   - Unsold vehicles with pending parts (9SEDAN, 10SUV)
--   - Unsold vehicles ready for sale (5SUV, 11SEDAN, 12TRUCK, 13COUPE)
--   - Empty vehicle types (Convertible, Minivan, Other, Van — no vehicles of these types)
--   - Empty condition (Very Good — only 11SEDAN uses it)
USE team15_database;
...
-- Vehicles (13 total)
-- Sold: 1SEDAN, 2SEDAN, 3SEDAN, 4TRUCK, 6COUPE, 7SUV, 8TRUCK (7 vehicles)
-- Unsold, ready for sale (no pending parts): 5SUV, 11SEDAN, 12TRUCK, 13COUPE (4 vehicles)
-- Unsold, parts pending: 9SEDAN (ordered bumper), 10SUV (ordered + received brakes) (2 vehicles)
INSERT INTO Vehicle (vin, model, `year`, `condition`, fuel_type, drive_train,
  ...
  ('9SEDAN004FAIR0000', 'Civic',     2021, 'Fair',      'Gas',     'FWD', 158, 'Waiting on new bumper', 'Honda',     'Sedan'),
  ('10SUV0003GOOD0000', 'Explorer',  2022, 'Good',      'Gas',     '4WD', 300, 'Pending brake repair',  'Ford',      'SUV'),
  ('11SEDAN05VGOOD000', '3 Series',  2023, 'Very Good', 'Gas',     'RWD', 255, 'One owner, garage kept', 'BMW',      'Sedan'),
  ('12TRUCK03GOOD0000', 'Tacoma',    2022, 'Good',      'Gas',     '4WD', 278, NULL,                    'Toyota',    'Truck'),
  ('13COUPE02EXCELLEN', 'Mustang',   2024, 'Excellent', 'Gas',     'RWD', 480, 'GT Premium package',    'Ford',      'Coupe');
...
INSERT INTO VehicleColor (vin, color_name) VALUES
  ...
  ('9SEDAN004FAIR0000', 'Silver'),
  ('10SUV0003GOOD0000', 'Blue'),
  ('11SEDAN05VGOOD000', 'Black'),
  ('12TRUCK03GOOD0000', 'White'),
  ('13COUPE02EXCELLEN', 'Red');
...
INSERT INTO Purchase (vin, purchase_price, purchase_date, seller_identifier, handler_username) VALUES
  ...
  ('9SEDAN004FAIR0000',  6000.00, '2025-07-01', 5, 'acq_frank'),
  ('10SUV0003GOOD0000', 20000.00, '2025-07-10', 6, 'acq_alice'),
  ('11SEDAN05VGOOD000', 22000.00, '2025-08-01', 1, 'acq_alice'),
  ('12TRUCK03GOOD0000', 28000.00, '2025-08-10', 2, 'acq_frank'),
  ('13COUPE02EXCELLEN', 38000.00, '2025-08-20', 3, 'acq_alice');
...
-- Vendors & Parts Orders
-- 8TRUCK: all parts Installed (sold vehicle, used for parts statistics report)
-- 4TRUCK: tires on order (sold vehicle, pending parts don't affect search since already sold)
-- 9SEDAN: bumper + paint kit on order (unsold, will appear as "parts pending")
-- 10SUV: brake caliper received + pads ordered (unsold, will appear as "parts pending")
...
INSERT INTO PartsOrder (parts_order_number, order_ordinal, vin, vendor_name) VALUES
  ...
  ('9SEDAN004FAIR0000-001', 1, '9SEDAN004FAIR0000', 'AutoParts Plus'),
  ('10SUV0003GOOD0000-001', 1, '10SUV0003GOOD0000', 'Quick Fix Supply');
...
INSERT INTO PartsOrderLine (vendor_part_number, description, status, quantity, unit_price, parts_order_number) VALUES
  ...
  ('AP-BMP-400', 'Front bumper',      'Ordered',  1, 450.00, '9SEDAN004FAIR0000-001'),
  ('AP-PNT-401', 'Bumper paint kit',  'Ordered',  1,  85.00, '9SEDAN004FAIR0000-001'),
  ('QF-BRK-600', 'Brake caliper set', 'Received', 1, 320.00, '10SUV0003GOOD0000-001'),
  ('QF-BRK-601', 'Brake pads rear',   'Ordered',  2,  55.00, '10SUV0003GOOD0000-001');
